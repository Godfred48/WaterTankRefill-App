...
@method_decorator((login_required, driver_required), name='dispatch')
class DriverDeliveryReportView(View):
    def get(self, request, *args, **kwargs):
        driver = request.user.driver_profile
        deliveries = Delivery.objects.filter(driver=driver).select_related('order', 'payment').order_by('-delivery_date')

        # Prepare HTTP response
        response = HttpResponse(content_type='application/pdf')
        response['Content-Disposition'] = 'attachment; filename="driver_delivery_report.pdf"'
        ...
        # Add Title
        elements.append(Paragraph("TankFill Report", title_style))
        elements.append(Spacer(1, 12))

        # Subtitle with driver name and date
        subtitle = f"Driver: {driver.user.full_name} | Date: {timezone.now().strftime('%B %d, %Y %H:%M')}"
        elements.append(Paragraph(subtitle, styles['Normal']))
        elements.append(Spacer(1, 20))

        # Table headers and rows
        data = [['Order ID', 'Customer', 'Vendor', 'Litres', 'Status', 'Delivery Date', 'Payment']]
        for delivery in deliveries:
            ...
            data.append([
                ...
                delivery.payment.payment_status if delivery.payment else "N/A"
            ])
        ...
        elements.append(Spacer(1, 20))
        total_deliveries = deliveries.count()
        delivered_count = deliveries.filter(delivery_status='Delivered').count()
        pending_count = total_deliveries - delivered_count
        total_payment = sum(d.payment.amount for d in deliveries if d.payment and d.payment.amount)

        summary = f"""
            <b>Total Deliveries:</b> {total_deliveries}<br/>
            <b>Delivered:</b> {delivered_count}<br/>
            <b>Pending:</b> {pending_count}<br/>
            <b>Total Payment:</b> GH₵ {total_payment:.2f}
        """
        elements.append(Paragraph(summary, styles['Normal']))

        # Footer note
        elements.append(Spacer(1, 30))
        footer = Paragraph("Generated by SammyKeys Tank Fill System", styles['Normal'])
...
@method_decorator((login_required, vendor_required), name='dispatch')
class VendorFullReportPDFView(View):
    def get(self, request, *args, **kwargs):
        vendor = request.user.vendor_profile
        orders = Order.objects.filter(vendor=vendor).order_by('-order_date')
        payments = Payment.objects.filter(order__vendor=vendor).order_by('-order__order_date')
        deliveries = Delivery.objects.filter(order__vendor=vendor).order_by('-delivery_date')

        response = HttpResponse(content_type='application/pdf')
        response['Content-Disposition'] = 'attachment; filename="vendor_full_report.pdf"'
        ...
        p.drawCentredString(width / 2, y, "Water Delivery App - Vendor Full Report")
        y -= 40

        # Vendor Info
        p.setFont("Helvetica", 11)
        p.drawString(50, y, f"Vendor: {vendor.user.full_name}")
        y -= 20
        p.drawString(50, y, f"Total Orders: {orders.count()} | Total Payments: {payments.count()} | Total Deliveries: {deliveries.count()}")
        y -= 30

        # Orders Section
        y = self.draw_section(p, y, "Orders Summary", orders, [
            ("Order ID", lambda o: str(o.order_id)),
            ("Customer", lambda o: o.customer.full_name),
            ("Status", lambda o: o.status),
            ("Date", lambda o: o.order_date.strftime("%Y-%m-%d")),
        ])

        # Payments Section
        y = self.draw_section(p, y, "Payments Summary", payments, [
            ("Payment ID", lambda pmt: str(pmt.payment_id)),
            ("Order ID", lambda pmt: str(pmt.order.order_id)),
            ("Amount", lambda pmt: f"${pmt.amount:.2f}"),
            ("Status", lambda pmt: pmt.payment_status),
            ("Date", lambda pmt: pmt.order.order_date.strftime("%Y-%m-%d")),  
        ])

        # Deliveries Section
        y = self.draw_section(p, y, "Deliveries Summary", deliveries, [
            ("Delivery ID", lambda d: str(d.delivery_id)),
            ("Order ID", lambda d: str(d.order.order_id)),
            ("Driver", lambda d: d.driver.user.full_name if d.driver else "N/A"),
            ("Status", lambda d: d.delivery_status),
            ("Delivered At", lambda d: d.delivery_date.strftime("%Y-%m-%d") if d.delivery_date else "Pending"),
        ])

        p.save()
        return response

    def draw_section(self, p, y, title, queryset, fields):
        width, height = letter
        if y < 120:
            p.showPage()
            y = height - inch
        ...
        x_positions = [50 + (i * 100) for i in range(len(fields))]

        for i, (label, _) in enumerate(fields):
            p.drawString(x_positions[i], y, label)
        y -= 15
        ...
        for obj in queryset:
            for i, (_, extractor) in enumerate(fields):
                p.drawString(x_positions[i], y, extractor(obj))
            y -= 15
            if y < 50:
                p.showPage()
                y = height - inch
