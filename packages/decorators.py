...
def vendor_required(view_func, denied_url="home"):
    @functools.wraps(view_func)
    def wrapper(request, *args, **kwargs):
        if (request.user.is_authenticated and request.user.is_vendor) or (request.user.is_authenticated and request.user.is_admin):
            return view_func(request, *args, **kwargs)
        messages.info(request, "Page not found")
        return redirect(denied_url)  
    return wrapper
...
def driver_required(view_func, denied_url="home"):
    @functools.wraps(view_func)
    def wrapper(request, *args, **kwargs):
        if (request.user.is_authenticated and request.user.is_driver) or (request.user.is_authenticated and request.user.is_admin):
            return view_func(request, *args, **kwargs)
        messages.info(request, "Page not found")
        return redirect(denied_url)  
    return wrapper
...
def customer_required(view_func, denied_url="home"):
    @functools.wraps(view_func)
    def wrapper(request, *args, **kwargs):
        if (request.user.is_authenticated and request.user.is_customer) or (request.user.is_authenticated and request.user.is_admin):
            return view_func(request, *args, **kwargs)
        messages.info(request, "Page not found")
        return redirect(denied_url)  
    return wrapper
